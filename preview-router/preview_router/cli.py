...
def detect_tailscale_ipv4() -> str | None:
    try:
        result = subprocess.run(["tailscale", "ip", "-4"], check=False, capture_output=True, text=True, timeout=5)
    except Exception:
        return None
...
def render_hosts_script(hosts: list[str], preview_ip: str) -> str:
    host_lines = "\n".join(f"  {shell_single_quote(host)}" for host in hosts)
    examples = "\n".join(f"echo \"  http://{host}/\"" for host in hosts[:5])
    if len(hosts) > 5:
        examples += "\necho \"  ...\""
    return f"""#!/usr/bin/env bash
set -euo pipefail

# Generated by `preview-router hosts-script`.
# Idempotently add local preview hostnames to /etc/hosts.
# Optional override:
#   PREVIEW_IP=100.x.y.z ./install-hosts.sh

PREVIEW_IP=\"${{PREVIEW_IP:-{preview_ip}}}\"
HOSTS=(
{host_lines}
)

START_MARKER=\"# >>> preview-router hosts >>>\"
END_MARKER=\"# <<< preview-router hosts <<<\"
TMP_FILE=\"$(mktemp)\"
...
{examples}
echo
echo \"Note: /etc/hosts maps names to IPs only; it cannot map ports or TLS certs.\"
"""
...
def cmd_hosts_script(args: argparse.Namespace) -> None:
    ...
    output.chmod(0o755)
    print(f"Wrote {output} for {preview_ip}: {', '.join(hosts)}")
...
def main(argv: list[str] | None = None) -> None:
    ...
    p_hosts = sub.add_parser("hosts-script", aliases=["generate-hosts-script"], help="generate an /etc/hosts installer from configured projects")
    p_hosts.add_argument("--output", "-o", default="install-hosts.sh", help="output path, or '-' for stdout")
